 ...
 *
 * A component that manages and displays tabulated quotes. The quotes can be filtered and searched based on suppliers and free text.
 * User can toggle between fulfilled and unfulfilled quotes based on activeTab state.
 * Fetches data using infinite scrolling.
 *
 * @module QuotesList
 * QuotesList functional component.
 *
 * @function QuotesList
 * @param {Object} props - React properties passed from parent
 * @prop {string} activeTab - State that toggles between fulfilled and unfulfilled quote listing.
 * @returns {ReactElements} JSX
 */

const QuotesList = ({ activeTab }) => {
  ...
  const fetchQuotes = ({ searchValue = "", nextPage = null } = {}) => {
    ...
    // Depending on the activeTab boolean - add the boolean to the request made to teh server to
    // fetch either fulfilled or unfulfilled quotes
    if (activeTab === "fulfilled") options.fulfilledFilter = true;

    getQuotes(token, setBaseQuotes, options).then((response) => {
