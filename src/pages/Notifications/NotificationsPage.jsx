...
import NotificationsTable from "../../components/Manufacturer/ManufacturerTable";
import { Spinner } from "react-bootstrap";
import debounce from "lodash/debounce";
import { getExpiryNotifications } from "../../clients/notifications_client";
const NotificationsPage = () => {
  ...
  const [viewNotifications, setViewNotifications] = useState([]); // State for manufacturers to display
  const [baseNotifications, setBaseNotifications] = useState([]); // Base state for all fetched manufacturers
  ...
  useEffect(() => {
    if (!baseNotifications.length) return; // Prevents the effect from running if there are no manufacturers
    extractEntitiesSelectList(
      baseNotifications,
  ...
  useEffect(() => {
    ...
    if (!supplier) {
      setViewNotifications([]); // Resets the viewNotifications if no supplier is selected
    }
  ...
  const fetchNotifications = ({ searchValue = "", nextPage = null } = {}) => {
    isLoadingRef.current = true; // Indicates the start of a data loading process
    getExpiryNotifications(token, setBaseNotifications, {
    ...
    <div>
      ...
      <Container className="my-3">
        ...
        <Row className="align-items-center justify-content-md-evenly">
          ...
          <Col md="auto" className="m">
            <ManufacturerModal onSuccessfulSubmit={fetchNotifications} />
            {/* ManufacturerModal triggers fetchNotifications on successful submit */}
          </Col>
      ...
      <InfiniteScroll
        ...
        loadMore={() => {
          ...
          {
            /* Triggers fetchNotifications for more data when scrolled to the bottom */
          }
        }}
        hasMore={hasMore}
        loader={
      ...
      >
        {/* NotificationsTable displays the list of manufacturers */}
        <NotificationsTable
          notificationsList={
            viewNotifications.length ? viewNotifications : baseNotifications
          }
